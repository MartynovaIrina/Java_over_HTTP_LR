...
;
	...
	int maxRandomNumberCities = Integer.parseInt(lr.eval_string("{CITIES_count}")); // or lr.eval_int("{CITIES_count}");
...
;
	...
	int numberOfBookings = Integer.parseInt(lr.eval_string("{FLIGHTID_count}")); //geting the number of FLIGHTID array parameters;
	...
	{
		randomBookingNumberToDeleteIndex = random.nextInt(numberOfBookings) + 1; //+1 cause we need the numeration starting from 1
	}
	
	String flightToDelete = String.format("{FLIGHTID_%d}", randomBookingNumberToDeleteIndex);
	lr.message(String.format(lr.eval_string("{CURRTIME} :   Booking to delete: %d booking %s"), 
	                         randomBookingNumberToDeleteIndex, lr.eval_string(flightToDelete)));
	
	//following is used to determine the flight index to confirm the deletion, and cyclically add all flightID values
	for (ñurrIndex = 1; ñurrIndex <= numberOfBookings; ñurrIndex++) {
		if (ñurrIndex == randomBookingNumberToDeleteIndex) {
			bodyReq += Integer.toString(ñurrIndex) + "=on&";
		}
		bodyReq += "flightID=";
		String currBooking = String.format("{FLIGHTID_%d}", ñurrIndex);
		bodyReq += lr.eval_string(currBooking) + "&";
	}
	
	if (numberOfBookings == 1) {
		bodyReq += "removeFlights.x=47&removeFlights.y=3&";
	}
	for (ñurrIndex = 1; ñurrIndex <= numberOfBookings; ñurrIndex++) {
		bodyReq += ".cgifields=" + Integer.toString(ñurrIndex);
	}
	if (numberOfBookings > 1) {
		bodyReq += "&removeFlights.x=47&removeFlights.y=3&";
	}
